...
int N;
ll vec[maxn];
ll last[maxn]; // 前一行的dp数组
ll dp[maxn]; // 本行的dp数组, dp[j]:从第1块路到当前路, 且当前路调整至高为Hash[j]的最低总消耗
ll solve(void) {
    memset(dp, 0x3f, sizeof(dp));
    ll ans = INF;
    for (int j = 1; j <= N; ++j) last[j] = abs(vec[1] - Hash[j]);
    for (int i = 2; i <= N; ++i) {
        ll last_cost = last[1]; // 前一块路高度调整至小于Hash[j]的高度所需的最小cost
        for (int j = 1; j <= N; ++j) {
            last_cost = min(last_cost, last[j]);
            dp[j] = abs(vec[i] - Hash[j]) + last_cost;
        }
        memcpy(last, dp, sizeof(dp));
    }
    for (int j = 1; j <= N; ++j) ans = min(ans, last[j]);
    return ans;
}
